 ...
 * FinishSetDialog
 *
 * Modal PrimeNG Dialog do zakończenia seta z opcjonalnymi uwagami trenera.
 */
@Component({
  selector: 'app-finish-set-dialog',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './finish-set-dialog.component.html',
  styleUrl: './finish-set-dialog.component.css',
})
export class FinishSetDialogComponent {
  // Props
  readonly visible = input.required<boolean>();
  readonly setNumber = input.required<number>();
  readonly currentScore = input.required<{ player: number; opponent: number }>();
